...
```
The `r max.number.steps` 5-minute interval contains the largest number of steps on average. 
...
```
The total number or missing values in the dataset is ```r missing.values```
...
options(scipen=999)
```
The mean total number of steps taken per day is `r new.mean.steps` and the median is `r new.median.steps`. 

Now, to answer the questions: 
* Do these values differ from the estimates from the first part of the assignment? 
* What is the impact of imputing missing data on the estimates of the total daily number of steps?

The mean total number of steps taken per day before imputing missing data was `r mean.steps`, and imputing missing data is `r new.mean.steps`.  
